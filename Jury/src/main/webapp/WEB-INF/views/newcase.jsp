...
function searchAjax() {
	...
	$.getJSON("/Jury/getsubcasetypes", { casetype: casetype }, function(result) {
...
<h3>Create New Case</h3>
    <form class="login-form" method="post" id="newcase" action="caseproceed">
   Select Case Type:  <select name="casetype" id="casetype" type="select" onchange="searchAjax()">
     	<option>    </option>
 		<c:forEach items="${caseTypes}" var="caseNames">
 		        	<option>${caseNames.casename}</option>
		</c:forEach>
     </select><br/><br/><br/>
         Select Sub Case: <select name="subcasetype" id="subcasetype" type="select">
     	
     </select><br/><br/>
     
     
      <input type="submit" onclick="return namevalidate(this)" name="submit" value="Proceed" style="width:120px;float:right;">
